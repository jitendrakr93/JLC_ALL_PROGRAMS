...
var ID;
function openAlert() {
	window.alert("You Clicked 4 Alert Box");
}

function openConfirm() {
	temp=window.confirm("You Clicked 4 Confirm Box(YES?NO)");
	msg=(temp)?"You Are Right":"You Are Wrong";
	window.status=msg;
...
function openNewWin() {
	NewWin=window.open('WinTest.html', 'NewWin', 'toolbar=no,status=no,width=300,height=300');
}
...
function formatTime() {
	...
	if(document.clock.jlctime[0].checked){
		...
		document.getElementById("jlc").innerHTML='<h1>'+((hour<=9)?"0"+hour:hour)+":"+min+":"+sec+add;
	}
